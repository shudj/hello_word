...
public class HanlpDemo1 {
    ...
    private static void participle() {
        ...
        for (Term t : termList) {
            ...
            //副食/n [1:3]
            //食品/n [2:4]
            System.out.println(t + " [" + t.offset + ":" + (t.offset + t.word.length()) + "]");
        }
        ...
        // [大衛貝克漢/nrf, 不僅僅/d, 是/v, 名著/n, 名/q, 球員/n, ，/w, 球場/n, 以外/f, ，/w,
        // 其/r, 妻/ng, 為/p, 前/f, 辣妹/nz, 合唱團/n, 成員/n, 維多利亞/ns, ·/w, 碧/ag, 咸/ng,
        // ，/w, 亦/d, 由於/c, 他/r, 擁有/v, 突出/a, 外表/n, 、/w, 百變/nz, 髮型/n, 及/c, 正面/d,
        // 的/uj, 形象/n, ，/w, 以至/c, 自己/r, 品牌/n, 的/uj, 男士/n, 香水/n, 等/u, 商品/n, ，/w,
        // 及/c, 長期/d, 擔任/v, 運動/n, 品牌/n, Adidas/nx, 的/uj, 代言人/n, ，/w, 因此/c, 對/p, 大眾/n,
        // 傳播/vn, 媒介/n, 和/c, 時尚界/nz, 等/u, 方面/n, 都/d, 具/vg, 很大/d, 的/uj, 影響力/n, ，/w, 在/p,
        // 足球/n, 圈外/nz, 所/u, 獲得/v, 的/uj, 認/v, 受/v, 程度/n, 可/v, 謂/vg, 前所未見/l, 。/w]
        System.out.println(fttermList);

        //极速词典分词
        //极速分词是词典最长分词，速度极其快，精度一般。调用方法如下:
        String text = "江西鄱阳湖干枯，中国最大淡水湖变成大草原";
        List<Term> speedList = SpeedTokenizer.segment(text);
        ...
        Segment shortSegment = new ViterbiSegment().enableCustomDictionary(false).enablePlaceRecognize(true).enableOrganizationRecognize(true);

        String[] testCase = {"刘喜杰石国祥会见吴亚琴先进事迹报告团成员", "江西鄱阳湖干枯，中国最大淡水湖变成大草原"};
        for (String sentence : testCase) {
            //N-最短分词：[刘喜杰/nr, 石国祥/nr, 会见/v, 吴亚琴/nr, 先进/a, 事迹/n, 报告团/n, 成员/n]
            //最短路分词: [刘喜杰/nr, 石国祥/nr, 会见/v, 吴亚琴/nr, 先进/a, 事迹/n, 报告团/n, 成员/n]
            System.out.println("\nN-最短分词：" + nShortSegment.seg(sentence) + "\n最短路分词: " + shortSegment.seg(sentence));
        }
        ...
        List<Term> crfTerms = crfSegment.seg("你看过穆赫兰道吗");
        System.out.println(crfTerms);

        for (Term term : crfTerms) {
            if (term.nature == null) {
    ...
    /**
     * 这里使用HanLP进行分词 ,只获取名称(Nature.n)和动名称(Nature.vn)
     * @param text
     * @return
     */
    private static List<String> hanLPSegment(String text) {
        ...
        for (Term term : segments) {
            if (term.nature == Nature.n || term.nature == Nature.vn) {
    ...
    private static void extractSummary() {
        ...
        // 返回的摘要长度
        String summary = HanLP.getSummary(document, 50);
        // 这类算法在有限的一段时间内终止。有限的非确定算法。这类算法在有限的时间内终止。
    ...
    private static void sugget_test() {
    ...
    private static void nameRecongnize() {

        System.out.println("中国人名识别: /nr");
        String[] testCase = {
        ...
        Segment organizationRecognize = HanLP.newSegment().enableOrganizationRecognize(true);
        for (String sentence : groupCase) {
            List<Term> seg = organizationRecognize.seg(sentence);
